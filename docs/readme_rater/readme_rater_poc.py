    ...
    for id_ in ids:
        ...
        if id_ == "CLARITY_OF_PURPOSE":
            ...
            advice = (
                ...
                if ok
                else "Add a one-paragraph purpose statement near the top."
            )
        elif id_ == "QUICKSTART_INSTALL":
            ok = present("pip install", "installation", "install\n", "install:")
            status = "pass" if ok else "fail"
            advice = (
                "Install command or link present."
                if ok
                else "Provide a pip install or clear install link."
            )
        elif id_ == "HELLO_WORLD_EXAMPLE":
            ok = present("```", "import ", "from ")
            status = "pass" if ok else "fail"
            advice = (
                "Minimal code example detected."
                if ok
                else "Add a short, copy-pasteable usage example."
            )
            ...
            if has_visual:
                ...
                status, advice = (
                    "fail",
                    "Provide a screenshot/GIF or sample output image.",
                )
            ...
            advice = (
                ...
                if ok
                else "Link CONTRIBUTING.md or add a contribution section."
            )
        elif id_ == "DEVELOPMENT_SETUP":
            ok = present("pytest", "make test", "uv run", "poetry run", "tox")
            status = "pass" if ok else "fail"
            advice = (
                "Dev/test setup is documented."
                if ok
                else "Document local dev setup and how to run tests."
            )
        elif id_ == "LICENSE_CLARITY":
            ok = present("license", "apache-2.0", "mit license", "bsd", "lgpl", "mpl")
            status = "pass" if ok else "fail"
            advice = (
                "License visible." if ok else "State the license or link to LICENSE."
            )
            ...
            advice = (
                ...
                if ok
                else "Briefly compare to similar tools or prior art."
            )
            ...
            advice = (
                ...
                if ok
                else "Explain key design choices and trade-offs."
            )
        else:
...
def qual_label(n: int) -> str:
    return (
        "Problematic"
        if n < 40
        else "Needs Improvement" if n < 70 else "Good" if n < 90 else "Excellent"
    )
...
def main(argv: List[str]) -> int:
    ...
    merged: Dict[str, Dict[str, str]] = {
        x["id"]: x for x in prev.get("rubric_results", [])
    }
